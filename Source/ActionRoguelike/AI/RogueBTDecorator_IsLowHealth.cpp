...
bool URogueBTDecorator_IsLowHealth::CalculateRawConditionValue(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory) const
{
	...
	if (ensure(ActionComp))
	{
		...
		float HealthFraction = ActionComp->GetAttributeValue(SharedGameplayTags::Attribute_Health) / ActionComp->
			GetAttributeValue(SharedGameplayTags::Attribute_HealthMax);
		return HealthFraction < LowHealthFraction; //(ActionComp->GetHealth() / ActionComp->GetHealthMax()) < LowHealthFraction;
	}
